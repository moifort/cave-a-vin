...
- **Global search** — full-screen search across names, producers, subtypes, regions, vintages and people (gifts, recommendations), ranked by relevance and grouped by match (in cellar, consumed, gifts…), with combinable facet chips. Words are matched independently and in any order, so "bordeaux margaux" and "margaux 2015" both land; every wine carries its searchable terms in a Firestore array, so a query reads the wines it returns rather than the whole cellar
