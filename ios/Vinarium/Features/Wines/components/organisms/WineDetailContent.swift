...
struct WineDetailContent: View {
    ...
    var body: some View {
        List {
            if let currentCellarPosition {
                WineCellarPositionSection(position: currentCellarPosition)
            }
    ...
    /// the position is history and only the cellar section states it.
    ...
    private var headerSubtitle: String {
        ...
        let fallback = content.beverageType == .wine
            ? content.color?.label
            : content.beverageType.label
        return fallback ?? ""
    }
}
